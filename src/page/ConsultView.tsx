...
const ConsultView = () => {
    ...
    const [viewData, setViewData] = useState<ConsultProps>();
    ...
    const DeleteData = async(id : string) => {
        try{
            ...
            if(ckDelete){
                await ConsultDelete(id, 'consult');
                setResult({
                    type : "success",
    ...
    useEffect(() => {
        const load = async() => {
            try{
                const result = await ConsultSelectOne(String(consult_id), 'consult');
                setViewData(result);
            }catch{
                alert("올바른 접근이 아닙니다.")
            }
    ...
    return(
        <div id="ConsultView" className='board-view'>
            {result && <ResultMessage title={"게시글 삭제"} type={result.type} message={result.message} onClose={() => setResult(null)} link={`/consult/list`} />}
            ...
            <ul className="info">
                ...
                <li><span>매물유형</span>{viewData.consult_type}</li>
                <li><span>가격유형</span>{viewData.consult_price}</li>
            </ul>
            <div className="content">{viewData.content}</div>
            <div className="btn">
                <Link to="/consult/list">글 목록</Link>
                <Link to={`/consult/write/${viewData.id}`} className="edit-btn">글 수정</Link>
